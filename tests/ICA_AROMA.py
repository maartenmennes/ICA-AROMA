...
optoptions.add_argument('-s', dest="rnseed", required=False, default=None, type=int)
...
if args.rnseed is not None:
	random.seed(args.rnseed)

print 'Step 1) MELODIC'
runICA(fslDir, inFile, outDir, melDir, mask, dim, TR, seed=args.rnseed)
...
register2MNI(fslDir, melIC, melIC_MNI, affmat, warp)

print '  - extracting the CSF & Edge fraction features'
edgeFract, csfFract = feature_spatial(fslDir, outDir, scriptDir, melIC_MNI)
